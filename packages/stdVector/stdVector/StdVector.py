...
##         return
    ...
    def __numarrayDims( self, dims):
        ...
        if not dims:
            dims = [ int( self.size())] # convert to int b/c size_t might be 8
        else:
            length = 1
            for dim in dims:
                length *= dim
            if length > self.size():
                msg = "specified dimensions (%s) exceed size of vector %s" %\
                      ( str(dims), self.size())
...
# version
__id__ = "$Id: StdVector.py 141 2007-07-13 21:40:11Z linjiao $"
