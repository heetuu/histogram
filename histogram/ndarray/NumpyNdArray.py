...
class NdArray(NdArrayBase):
    ...
    def __init__( self, datatype, arg2, initVal=0, **kwds):
        ...
        elif datatype in ["float", "double", "int", "uint"] : t = numpytypecode_from_typename(datatype)
        else: raise NotImplementedError , "unknow type %s" % datatype
    ...
    def datatype(self):
        ...
        except: return getNumericArray_aktypecode( arr )
...
def numpytypecode_from_typename( name ):
    ...
    elif name == "float": return "float32"
    elif name == "int": return 'int32'
    elif name == "uint": return "uint32"
    else: raise NotImplementedError , "unknow type code %s" % name
    raise
